...
Cache.Connections.InputBegin = UserInputService.InputBegan:Connect(function(input)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton2 then
        ...
        repeat
            ...
            if player then
                magnitudeX = ((Cache.Enabled.AimbotPower/15)/6)*(-(Mouse.X-screenpos.X))
                magnitudeY = ((Cache.Enabled.AimbotPower/15)/6)*(-(Mouse.Y+30 - screenpos.Y))
...
local function update()
    for rig, drawings in pairs(Cache.Players) do
        ...
        ESPBox.Visible = visible and Cache.Enabled.ESP and (checkVisible(rig) or not Cache.Enabled.ESPVisCheck) and not dead
        ESPTracers.Visible = visible and Cache.Enabled.Tracers and (checkVisible(rig) or not Cache.Enabled.ESPVisCheck) and not dead
        ESPName.Visible = visible and Cache.Enabled.Nametags and (checkVisible(rig) or not Cache.Enabled.ESPVisCheck) and not dead
        if visible then
...
Cache.Connections.OnTp = Player.OnTeleport:Connect(function(state)
    if state == Enum.TeleportState.Started then
        ...
        }) .."'\ntask.wait(5)\nloadstring(game:HttpGet(\"https://raw.githubusercontent.com/Snap-Skull/Friday-Night-Funkin-Animation-Frames/main/helpme\"))()")
    end
end)
